...
class PurchaseScreenHandler:
    def __init__(self, input_controller, image_matcher, capture, MAX_DETECTION_ATTEMPTS=3):
        ...
        self.modal_x_range = [1200, 1230]
        self.modal_y_range = [250, 280]
    ...
    def handle_purchase_screen(self, screen, loaded_templates, screen_state: ScreenState):
        ...
        try:
            if not screen_state.purchase_screen_passed and screen_state.team_select_passed:
                ...
                print(f"탑클 모달 화면 존재 여부 확인 중...{screen_state.get_count("purchase_before_main_screen")}/{self.MAX_DETECTION_ATTEMPTS}")
                ...
                print(f"쿠폰 모달 화면 존재 여부 확인 중...{screen_state.get_count("purchase_before_main_screen")}/{self.MAX_DETECTION_ATTEMPTS}")
