...
class MainScreenHandler:
    def __init__(self, image_matcher, capture, MAX_DETECTION_ATTEMPTS=5, MAX_PCICON_DETECTION_ATTEMPTS=2):
        ...
        try:
            if not screen_state.main_screen_passed and screen_state.purchase_screen_passed:
                ...
                if not screen_state.pc_icon_passed:
                    ...
                    top_left, bottom_right, _ = self.image_matcher.detect_template(screen, loaded_templates['pc_icon_bar'], threshold=0.8)
                    if top_left is None or bottom_right is None:
                        return False
                    roi = (top_left[0], top_left[1] - 52, bottom_right[0], top_left[1])
                    if self.image_matcher.process_template(screen, 'pc_icon', loaded_templates, threshold=0.8, roi=roi):
